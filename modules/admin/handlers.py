...
class AdminModule(BotModule):
    ...
    def _setup_handlers(self):
        ...
        async def show_stats_handler(message: Message, bot_id: int = None):
            if not config.is_admin(message.from_user.id) or not bot_id: return
            stats = await get_stats()
            participants = await get_participants_count()
            conversion = (participants / stats['total_users'] * 100) if stats['total_users'] else 0
            stats_msg = config_manager.get_message('stats_msg', "📊 Статистика\n\n👥 Пользователи: {users} (+{users_today})\n🧾 Чеки: {receipts} (принято: {valid}) (+{receipts_today})\n🎯 Участников: {participants}\n📈 Конверсия: {conversion:.1f}%\n🏆 Победителей: {winners}", bot_id=bot_id).format(users=stats['total_users'], users_today=stats['users_today'], receipts=stats['total_receipts'], valid=stats['valid_receipts'], receipts_today=stats['receipts_today'], participants=participants, conversion=conversion, winners=stats['total_winners'])
        ...
        async def process_broadcast_content(message: Message, state: FSMContext, bot: Bot, bot_id: int = None):
            if not config.is_admin(message.from_user.id): return
        ...
        # ... Raffle state handlers (omitted for brevity in this step, but would be fully implemented) ...
        # For brevity, I'll implement the rest of the Admin logical blocks now.
        ...
        async def raffle_count(message: Message, state: FSMContext, bot_id: int = None):
            ...
            participants = await get_participants_count() if bot_id else 0
        ...
        async def raffle_confirm(message: Message, state: FSMContext, bot_id: int = None):
            if message.text != "✅ Подтвердить":
                ...
                return
            data = await state.get_data()
            campaign_id = await add_campaign("raffle", {"prize": data["prize"], "count": data["count"], "win_msg": data["win_msg"], "lose_msg": data["lose_msg"]}, data.get("scheduled_for"))
        ...
        async def show_winners(message: Message, bot_id: int = None):
            ...
            for c in campaigns:
                ...
                for w in c['winners'][:5]: text.append(f"  {'✓' if w['notified'] else '..'} {w.get('full_name', 'Unknown')}\n")
        ...
        async def export_winners_handler(message: Message, bot_id: int = None):
            ...
            try:
                with os.fdopen(fd, 'w', encoding='utf-8-sig', newline='') as f:
                    ...
                    for w in winners: writer.writerow([w.get('full_name', ''), w.get('phone', ''), f"@{w.get('username', '')}", w.get('prize_name', ''), str(w.get('created_at', ''))[:19]])
        ...
        async def process_manual_user(message: Message, state: FSMContext, bot_id: int = None):
            if message.text == "❌ Отмена":
                ...
                await message.answer("Отменено", reply_markup=get_main_keyboard(True, bot_manager.bot_types.get(bot_id, 'receipt')))
                return
            user = await search_users(message.text.strip())
            user = user[0] if user else (await get_user_by_id(int(message.text)) if message.text.isdigit() else None)
            if not user:
            ...
            await message.answer(f"⚠️ Добавить чек для {user['full_name']}?", reply_markup=get_confirm_keyboard())
        ...
        async def confirm_manual_receipt(message: Message, state: FSMContext, bot_id: int = None):
            if message.text != "✅ Подтвердить":
                ...
                return
            data = await state.get_data()
            receipt_id = await add_receipt(user_id=data['user_id'], status="valid", data={"manual": True}, fiscal_drive_number="MANUAL", fiscal_document_number=f"M_{int(time.time())}", fiscal_sign="MANUAL", total_sum=0, raw_qr="manual", product_name="Ручное добавление")
            await message.answer(f"✅ Чек #{receipt_id} добавлен!", reply_markup=get_main_keyboard(True, bot_manager.bot_types.get(bot_id, 'receipt')))
            await state.clear()
