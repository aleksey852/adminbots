...
async def show_stats(message: Message, bot_id: int = None):
    if not config.is_admin(message.from_user.id):
        return
    if not bot_id:
        await message.answer("Ошибка: бот не определен")
        return
...
async def start_broadcast(message: Message, state: FSMContext, bot_id: int = None):
    ...
    total = await get_total_users_count(bot_id)
...
async def show_winners(message: Message, bot_id: int = None):
    ...
    campaigns = await get_recent_raffles_with_winners(bot_id, limit=5)
    if not campaigns:
...
async def export_winners(message: Message, bot_id: int = None):
    if not config.is_admin(message.from_user.id):
        return
    if not bot_id: return
    
    winners = await get_all_winners_for_export(bot_id)
    if not winners:
